...
const NavItems = ({ ToggleMenu, MenuOpen, isActive, }) => {
  ...
  return (
    <>
      <nav className={`${MenuOpen ? 'translate-x-0' : '-translate-y-full'} duration-300 mx-auto text-white  poppins-medium bg-[#2AA11F]  min-h-65 max-h-screen max-w-full w-full top-0 right-0 absolute z-50`}>
          ...
          <ul className="flex flex-col items-start mt-5 *:my-2 *:px-2  overflow-y-auto max-h-[calc(100vh-150px)] ">
            ...
            <li ref={dropdown1Ref} >
              <Link onClick={() => setDropdown1(!dropdown1)} className={`flex items-center gap-1  ${about.some(link => location.pathname === link.path)
                ? 'text-[#2AA11F] bg-white   py-1 px-1 rounded-lg'
                : ''
                }`}>
                About Us
                <FaChevronDown
              ...
              {dropdown1 && (
                <div className="border-2 border-white top-9 rounded-lg mt-1">
            ...
            <Link to="/contact-us" className={` ${isActive == '/contact-us' ? 'text-white bg-[#2AA11F]  rounded-lg min-w-[75px] py-1  px-2' : ''}`}>
              <li
                onClick={() => {
                  ToggleMenu(false);
